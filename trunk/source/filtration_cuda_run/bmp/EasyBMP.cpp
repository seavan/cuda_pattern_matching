...
RGBApixel cppBMP::GetPixel( int i, int j ) const
{
 using namespace std;
 bool Warn = false;
 if( i >= Width )
...
bool cppBMP::SetPixel( int i, int j, RGBApixel NewPixel )
{
 Pixels[i][j] = NewPixel;
 return true;
}
...
bool cppBMP::SetColor( int ColorNumber , RGBApixel NewColor )
{
 using namespace std;
 if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 )
 {
  if( EasyBMPwarnings )
  {
   cout << "EasyBMP Warning: Attempted to change color table for a cppBMP object" << endl
...
// RGBApixel cppBMP::GetColor( int ColorNumber ) const
RGBApixel cppBMP::GetColor( int ColorNumber )
{ 
 RGBApixel Output;
 Output.Red   = 255;
 Output.Green = 255;
 ...
 if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 )
 {
  if( EasyBMPwarnings )
  {
   cout << "EasyBMP Warning: Attempted to access color table for a cppBMP object" << endl
        << "                 that lacks a color table. Ignoring request." << endl;
  }
  return Output;
 }
...
// cppBMP::cppBMP( const cppBMP& Input )
cppBMP::cppBMP( cppBMP& Input )
{
...
RGBApixel* cppBMP::operator()(int i, int j)
{
 using namespace std;
 bool Warn = false;
 if( i >= Width )
...
// int cppBMP::TellBitDepth( void ) const
int cppBMP::TellBitDepth( void )
{ return BitDepth; }

// int cppBMP::TellHeight( void ) const
int cppBMP::TellHeight( void )
{ return Height; }

// int cppBMP::TellWidth( void ) const
int cppBMP::TellWidth( void )
{ return Width; }

// int cppBMP::TellNumberOfColors( void ) const
int cppBMP::TellNumberOfColors( void )
{
...
bool cppBMP::SetBitDepth( int NewDepth )
{
 using namespace std;
...
bool cppBMP::SetSize(int NewWidth , int NewHeight )
{
 using namespace std;
...
bool cppBMP::WriteToFile( const char* FileName )
{
 using namespace std;
...
bool cppBMP::ReadFromFile( const char* FileName )
{ 
 using namespace std;
 if( !EasyBMPcheckDataSize() )
 {
 ...
 if( !IsBitmap ) 
 {
  if( EasyBMPwarnings )
  {
   cout << "EasyBMP Error: " << FileName 
        << " is not a Windows cppBMP file!" << endl; 
  }
  fclose( fp ); 
  return false;
 }
...
bool cppBMP::CreateStandardColorTable( void )
{
 using namespace std;
 if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 )
 {
...
void cppBMP::SetDPI( int HorizontalDPI, int VerticalDPI )
{
 XPelsPerMeter = (int) ( HorizontalDPI * 39.37007874015748 );
 YPelsPerMeter = (int) (   VerticalDPI * 39.37007874015748 );
}

// int cppBMP::TellVerticalDPI( void ) const
int cppBMP::TellVerticalDPI( void )
{
 if( !YPelsPerMeter )
...
// int cppBMP::TellHorizontalDPI( void ) const
int cppBMP::TellHorizontalDPI( void )
{
 if( !XPelsPerMeter )
...
void PixelToPixelCopy( cppBMP& From, int FromX, int FromY,  
                       cppBMP& To, int ToX, int ToY)
{
 *To(ToX,ToY) = *From(FromX,FromY);
 return;
}

void PixelToPixelCopyTransparent( cppBMP& From, int FromX, int FromY,  
                                  cppBMP& To, int ToX, int ToY,
...
void RangedPixelToPixelCopy( cppBMP& From, int FromL , int FromR, int FromB, int FromT, 
                             cppBMP& To, int ToX, int ToY )
{
...
void RangedPixelToPixelCopyTransparent( 
     cppBMP& From, int FromL , int FromR, int FromB, int FromT, 
     cppBMP& To, int ToX, int ToY ,
     RGBApixel& Transparent )
{
 // make sure the conventions are followed
...
bool CreateGrayscaleColorTable( cppBMP& InputImage )
{
 using namespace std;
...
bool cppBMP::Read32bitRow( ebmpBYTE* Buffer, int BufferSize, int Row )
{ 
 int i;
...
bool cppBMP::Read24bitRow( ebmpBYTE* Buffer, int BufferSize, int Row )
{ 
 int i;
...
bool cppBMP::Read8bitRow(  ebmpBYTE* Buffer, int BufferSize, int Row )
{
 int i;
 if( Width > BufferSize )
 { return false; }
...
bool cppBMP::Read4bitRow(  ebmpBYTE* Buffer, int BufferSize, int Row )
{
 int Shifts[2] = {4  ,0 };
 int Masks[2]  = {240,15};
...
bool cppBMP::Read1bitRow(  ebmpBYTE* Buffer, int BufferSize, int Row )
{
...
bool cppBMP::Write32bitRow( ebmpBYTE* Buffer, int BufferSize, int Row )
{ 
 int i;
 if( Width*4 > BufferSize )
 { return false; }
...
bool cppBMP::Write24bitRow( ebmpBYTE* Buffer, int BufferSize, int Row )
{ 
 int i;
 if( Width*3 > BufferSize )
 { return false; }
...
bool cppBMP::Write8bitRow(  ebmpBYTE* Buffer, int BufferSize, int Row )
{
 int i;
 if( Width > BufferSize )
 { return false; }
...
bool cppBMP::Write4bitRow(  ebmpBYTE* Buffer, int BufferSize, int Row )
{ 
...
bool cppBMP::Write1bitRow(  ebmpBYTE* Buffer, int BufferSize, int Row )
{ 
...
ebmpBYTE cppBMP::FindClosestColor( RGBApixel& input )
{
 using namespace std;
...
bool Rescale( cppBMP& InputImage , char mode, int NewDimension )
{
 using namespace std;
 int CapMode = toupper( mode );

 cppBMP OldImage( InputImage );
